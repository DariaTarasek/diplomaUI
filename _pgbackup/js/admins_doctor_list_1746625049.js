...
createApp({
  ...
  function openEdit(s) {
    ...
    watch(() => form.firstName, (val) => {
        if (val.length === 0) firstNameError.value = 'Имя не может быть пустым'
        else firstNameError.value = '';
    });

    watch(() => form.secondName, (val) => {
        if (val.length === 0) secondNameError.value = 'Фамилия не может быть пустой'
        else secondtNameError.value = '';
    });
