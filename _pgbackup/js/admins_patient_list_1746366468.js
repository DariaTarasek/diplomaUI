...
createApp({
  setup() {
    ...
    function updateRequestButtonState() {
        ...
        requestButtonDisabled.value = !(digits.length === 11 && digits.startsWith('7'));
        }
  ...
  return digits.length === 11 && digits.startsWith('7');
    ...
    if (digits.length !== 11 || !digits.startsWith('7')) {
        codeMessage.value = 'Введите корректный номер телефона (11 цифр, начинается с 7).';
        return;
    }
    ...
    function openEdit() {
      ...
      // Маска уже в этом окне есть, поэтому инициализируется позже по таймеру
      setTimeout(() => {
        ...
        if (phoneInput) {
          ...
          digits = digits.slice(1); // убираем +7 / 8
