...
createApp({
  ...
  function openEdit(s) {
    ...
    const isSurnameOnlySpaces = form.surname && form.surname.trim() === '';

    if (isSecondNameEmpty) {
        secondNameError.value = 'Фамилия не может быть пустой или состоять только из пробелов';
    }
    if (isFirstNameEmpty) {
        nameError.value = 'Имя не может быть пустым или состоять только из пробелов';
    }
    if (isSurnameOnlySpaces) {
        surnameError.value = 'Отчество не может состоять только из пробелов';
    }

    if (isSecondNameEmpty || isFirstNameEmpty || isSurnameOnlySpaces) {
