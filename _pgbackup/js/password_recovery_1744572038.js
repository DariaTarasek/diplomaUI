document.addEventListener('DOMContentLoaded', () => {
  ...
  // Если есть буквы или символ @ — считаем это email и не маскируем
  if (/[a-zA-Z@]/.test(value)) return;

  // Если это что-то, похожее на телефон — применим маску
  const digits = value.replace(/[^\d]/g, '').substring(0, 11);
  let formatted = '';

  if (digits.startsWith('8')) {
    formatted = '8';
  } else if (digits.startsWith('7') || digits.startsWith('9')) {
    formatted = '+7';
  }
